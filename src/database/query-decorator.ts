...
import BeanFactory from "../../src/bean-factory.class";
...
function Select(sql: string) {
  return (target, propertyKey: string, descriptor: PropertyDescriptor) => {
    descriptor.value = async (...args: any[]) => {
      ...
      let sqlValues = [];
      if (args.length > 0) {
        [newSql, sqlValues] = convertSQLParams(
          args,
          target,
          propertyKey,
          newSql
      ...
      if (
        cacheBean &&
        cacheDefindMap.has([target.constructor.name, propertyKey].toString())
      ) {
